...
# 来源数据库列表
sourceDBnamelist = "owms_us_east::owms_ussc::owms_us_west"
hmsSourceDBnamelist = "ts_usea::ts_ussc::ts_us_west"
# 海外仓id
warehouseList = "1::23::4"
...
for i in range(0, len(sourceDBnamelist.split("::"))):
    ...
    zone_sql = """select case when saving_time=1 then (7-timezone) else (8-timezone) end as time_zone, 
                    ...
                    from warehouse w  
                    join wh_cut_off_time wc 
                      on wc.warehouse_code =w.warehouse_code ;""" % warehouse_date
    print(zone_sql)
    ...
    if dayofweek < 6:
        ...
        # 表一：同步owms订单表
        owms_dstable = 'temp_owms_orders_2'
        # 1、删除目标表中当前仓库的数据
        ...
        query_sql = (
                """select 
                      warehouse_id,wp_code,o.order_code,is_fba,order_status,sm_code,o.sc_code,'' as sort_code , 
        ...
        query_sql2 = (
                    """select %s as warehouse_id,cd.order_number as order_code,
                       ...
                       owms_warehouseid, dtyesterday_local, warehouse_date))
        print(query_sql2)
        # 结果保存在字典query_df中
        ...
        unfinish_dstable = 'temp_owms_orders_nottrans_2'
        # 1、删除目标表中当前仓库的数据
                    ...
                    """select o.warehouse_id, wp_code,is_fba,o.sm_code,o.sc_code, 
                         ...
                         from 
                              (select o.warehouse_id,is_fba,o.sm_code,o.sc_code, 
                                 ...
                                 and sm_code not in ('PICKED_UP_BARTER','PICKED_UP_DESTROY','SHANG_PIN_CHAI_FEN','SHANG_PIN_HE_BING','TUIJIANZIXUAN','WEIYUBAOTUIJIAN','ZITI')  
                              ) o 
        ...
        # 表四：目标表
        dstable = 't_ret_order_transport_sum_2'
        # 1、删除目标表中当前仓库的数据
                       ...
                       select '%s' as dtdate,date_sub(now(),interval %s hour) as create_time,
                         ...
                         from 
                              (select o.warehouse_id,wp_code,is_fba,sm_code,sc_code,
                                      count(distinct o.order_code) as owms_cnt,
                                 ...
                                 from temp_owms_orders_2 o
                                 left join temp_hms_container_details_2 c on o.order_code=c.order_code  and o.warehouse_id=c.warehouse_id
                              ...
                              select warehouse_id,wp_code,is_fba,sm_code,sc_code,
                                ...
                                from temp_owms_orders_nottrans_2
