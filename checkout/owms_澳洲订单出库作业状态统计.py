...
# 目标数据库
dbdest = 'fbg_mid_dw'
dstable = 't_ret_order_status_track_sum'
# 构建目标数据库连接字典
conn_dest = dict()
...
zone_sql = ("""select case when saving_time = 1 then (7-timezone) 
                      else (8-timezone) end as time_zone, 
...
dttoday = str(today)  # 统计日期后一天

dtdate_cut = str(beijing_cut)  # 统计当天北京截单时间
dtyesterday_cut = str(yesterday)  # 统计前一天北京截单时间
